...
export default function AdminLayout({
  ...
  return (
    <div className="flex h-screen bg-[#F5F0FA] overflow-hidden" dir="rtl">
      {/* Desktop Sidebar */}
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-40 lg:hidden">
          ...
          <div className="absolute right-0 top-0 h-full">
            <AdminSidebar onToggle={() => setMobileOpen(false)} />
          </div>
        </div>
      )}
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        {/* Top Bar */}
        <header className="bg-white border-b border-[#E5D9F2] px-4 md:px-6 h-[60px] flex items-center justify-between flex-shrink-0 gap-3">
          <div className="flex items-center gap-3">
            <button
              onClick={() => setMobileOpen(true)}
              className="lg:hidden w-9 h-9 rounded-xl bg-[#F0EBF8] flex items-center justify-center text-[#6B7280]"
            >

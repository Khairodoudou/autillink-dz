...
import { Calendar, Clock, CheckCircle, XCircle, Circle, MapPin, FileText, Check, Search } from "lucide-react";
import PageHeader from "@/components/ui/PageHeader";
import TablePagination from "@/components/ui/TablePagination";
import { mockSpecialistAppointments } from "@/lib/mock-data";
import type { SpecialistAppointment } from "@/lib/mock-data";
...
export default function SpecialistAppointmentsPage() {
  const [appointments, setAppointments] = useState<SpecialistAppointment[]>(mockSpecialistAppointments);
  ...
  const handleStatusChange = (id: string, newStatus: "مكتمل" | "ملغى") => {
  ...
  const filtered = appointments.filter((a) => {
    const matchSearch = a.patientName.includes(search) || (a.notes && a.notes.includes(search));
  ...
  const todayAppts = paginated.filter((a) => a.date === today && a.status === "قادم");
  const upcoming = paginated.filter((a) => a.date > today && a.status === "قادم");
  const past = paginated.filter((a) => a.status !== "قادم" || a.date < today);

  const renderCard = (apt: SpecialistAppointment, compact = false) => {
    ...
    return (
      <div
        ...
        className={`bg-white rounded-2xl border p-5 shadow-sm transition-all ${
          compact ? "border-[#D6E8F0] opacity-75" : "border-[#1D5B79]/20 border-2 hover:shadow-md"
        }`}
      >
          ...
          <div className="flex items-start gap-4">
            {!compact && (
              ...
              >
                ...
                <span className="text-[10px] font-700" style={{ color: typeColor }}>
            ...
            <div>
              <div className="flex items-center gap-2 mb-1">
                ...
                <span
                  className="flex items-center gap-1 text-[11px] font-700 px-2 py-0.5 rounded-full"
              ...
              <p className="text-base font-800 text-[#1F2937]">{apt.patientName}</p>

              <div className="flex flex-wrap gap-3 mt-1.5 text-xs text-[#9CA3AF]">
              ...
              {apt.notes && <p className="text-xs text-[#6B7280] mt-2 bg-[#F5F9FB] p-2 rounded-lg">{apt.notes}</p>}
            </div>
          </div>
          ...
          {apt.status === "قادم" && (
            <div className="flex items-center gap-2 flex-shrink-0 pt-2 sm:pt-0 border-t sm:border-t-0 border-[#EAF3F7]">
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-2 bg-[#2E8B7E] text-white text-xs font-800 rounded-xl hover:bg-[#22685e] transition-colors shadow-sm active:scale-95"
              >
              ...
              <Link
                ...
                className="flex items-center gap-1.5 px-3 py-2 bg-[#1D5B79]/10 text-[#1D5B79] text-xs font-800 rounded-xl hover:bg-[#1D5B79]/20 transition-colors"
              >
              ...
              <button
                ...
                className="px-2.5 py-2 text-xs font-700 text-red-500 hover:bg-red-50 rounded-xl transition-colors"
              >
                إلغاء
              </button>
            </div>
  ...
  return (
    <div>
      ...
      <div className="space-y-6">
        ...
        {todayAppts.length > 0 && (
          <div>
            <h2 className="text-sm font-800 text-[#1F2937] uppercase tracking-wide mb-4 flex items-center gap-2">
              <span className="w-2 h-2 rounded-full bg-[#1D5B79]" />
        ...
        {filtered.length === 0 && (
          <div className="bg-white rounded-2xl border border-[#D6E8F0] p-8 text-center">
            <Calendar className="w-10 h-10 text-[#9CA3AF] mx-auto mb-2" />
            <p className="text-sm text-[#6B7280]">لا توجد نتائج مطابقة للبحث أو التصفية</p>
          </div>
        )}
